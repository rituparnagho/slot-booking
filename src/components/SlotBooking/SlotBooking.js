...
const SlotBooking = () => {
  ...
  const handleBookSlot = () => {
    if (selectedDate && selectedTime) {
      ...
      const isSlotAlreadyBooked = bookedSlots.some(
        (existingSlot) =>
          existingSlot.date === selectedDate && existingSlot.time === selectedTime
      );
  ...
  const handleCancelBooking = (index) => {
    ...
    const savedBookedSlots = JSON.parse(localStorage.getItem('bookedSlotsAll')) || [];
    const updatedSavedSlots = savedBookedSlots.filter(slot => !(slot.date === cancelledSlot.date && slot.time === cancelledSlot.time));
  ...
  return (
    <div className="container">
      {loggedInUser ? (
                ...
                <div className="slot-info">
                  ...
                  <div className="slot-actions">
                    {slot.user === loggedInUser && (
                      <button className="cancel-button" onClick={() => handleCancelBooking(index)}>
                        Cancel Booking
                      </button>
                    )}
                  </div>
        ...
        <h1>Please log in to use the Slot Booking App <Link to="/">Login</Link></h1>
      )}
    </div>
  );
}
